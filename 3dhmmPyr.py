...
def get_dispute(inframe,outframe):
    diff_vec=np.sum(np.sum(np.power(np.abs(np.array(inframe,dtype=np.uint8)-np.array(outframe,dtype=np.uint8)),2),2),0)
    #diff_vec=np.reshape(diff_vec,diff_vec.shape[0])
    diff_vec=np.convolve(diff_vec,bad_gaussian,mode='same')
    m=np.argmax(diff_vec)
    return (m-bad_width/2,m+bad_width/2)
